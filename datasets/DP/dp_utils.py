...
from .text_to_dict import text_to_dict

names_mapping = {
  'result_scaled_camera_pose_bottom.txt':'result_scaled_image_bottom.jpg',
  'result_scaled_camera_pose_top.txt':'result_scaled_image_top.jpg',
  'result_scaled_camera_pose_left.txt':'result_scaled_image_left.jpg',
  'result_scaled_camera_pose_right.txt':'result_scaled_image_bottom.jpg',
  'result_scaled_camera_pose_center.txt':'result_scaled_image_center.jpg',
}
...
def ReadView(base_dir, view_json):
  return View(
      ...
      shape=(int(view_json['size_y']), int(view_json['size_x'])),
      camera=Camera(
          _IntrinsicsFromViewDict(view_json),
          _WorldFromCameraFromViewDict(view_json)))


def ReadScene(base_dir, scene_name):
  """Reads a scene from the directory base_dir."""
  list_camera_files = os.listdir(base_dir)
  
  camera_dicts = []
  camera_views = []
  for camera_name in list_camera_files:
    cam_file = os.path.join(base_dir,camera_name)
    cam_dict = text_to_dict(cam_file)
    cam_dict['relative_path'] = os.path.join(base_dir,'../../scaled_images/'+scene_name)
    cam_dict['relative_path'] = os.path.join(cam_dict['relative_path'],names_mapping[camera_name])
    camera_views.append(ReadView(base_dir, cam_dict))
    camera_dicts.append(cam_dict)

  # all_views = []
  # for views in model_json:
  #   all_views.append([ReadView(base_dir, view_json) for view_json in camera_dicts])
  return camera_views, camera_dicts
